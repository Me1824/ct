...
if(isset($_GET["id"]))
$sql=mysql_query("update article set num=num+1 where id=".$_GET["id"]);
?>
<body style="background-color: rgb(242,242,242);">
	...
	<div class="middle_1">
		...
		<div class="box6">
			<?php
                $sql=mysql_query("select * from article,user where article.id=$_GET[id] and article.author=user.id");
                $row=mysql_fetch_object($sql);
                ?>
			<div class="zuobian_1"> 
				<span style="top:28px;"><?php echo $row->title?></span>
				<h5>发布于<?php echo $row->uname?>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $row->datetime?></h5>

				<span style="font-size:18px;line-height:40px;"><?php echo $row->content?></span>
			</div>
